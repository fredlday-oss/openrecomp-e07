...
def main(argv: list[str]) -> int:
    if len(argv) != 5:
        print("usage: run_aot_mips32_expansion_v1.py <native-module> <ir.json> <fixture.json> <out-result.json>", file=sys.stderr)
        return 2
    try:
        ...
        if architecture not in {"mips32-le", "mips32-be"}:
            raise ValueError("expanded MIPS32 AOT proof requires mips32-le or mips32-be normalized source")
        if architecture != meta["architecture"]:
            raise ValueError("fixture/normalized architecture mismatch")
        if ir["required_host_symbols"]:
            raise ValueError("expanded MIPS32 AOT proof unexpectedly requires host calls")
        ...
        byteorder = ir["source"]["endianness"]
        result = {
        ...
        print(f"OPENRECOMP_MIPS32_EXPANSION_AOT=FAIL: {exc}", file=sys.stderr)
        return 2

    print(f"MIPS32_EXPANSION_AOT_V0={result['return_v0']}")
    print(f"MIPS32_EXPANSION_AOT_CHECKSUM={result['checksum']}")
    print(f"MIPS32_EXPANSION_AOT_OPERATIONS={result['operations']}")
    print("OPENRECOMP_MIPS32_EXPANSION_AOT=PASS")
    return 0
