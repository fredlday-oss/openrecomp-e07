...
| Private TMNT compatibility observation | **UNPROVEN** | MMC1 ingestion/platform construction succeeds, but translation stops at opcode byte `0x7C` at `0xC570`, three unresolved `jmp ($E2)` sites and the bank-switched low-window frontier; native execution not reached |
...
The private TMNT image remains a compatibility observation only. Phase 6 superseded the earlier MMC1 mapper blocker, but the current frontier remains translation/control-flow: opcode byte `0x7C` at `0xC570`, unresolved `jmp ($E2)` sites at `0x86E8`, `0x8956` and `0x8F3C`, and 1,048 candidate instructions in the bank-switched `$8000-$BFFF` window. Native execution and meaningful interactive behavior have not been demonstrated for this private title, so playability remains **UNPROVEN**.
