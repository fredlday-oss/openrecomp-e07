...
| Phase-5 NES/NROM platform proof | **PROVEN — bounded** | Original Apache-2.0 NROM fixture completed ingestion -> decode/semantics -> neutral structure -> runtime -> generated host-native execution -> independent reference equivalence |
| Phase-6 NES/MMC1 platform proof | **PROVEN — bounded** | Original Apache-2.0 MMC1 fixture validated the required serial-register, PRG/CHR banking and mirroring behavior through generated native execution and independent reference equivalence |
| Phase-6 ROM-to-native workflow | **PASS — bounded** | Deterministic supported-input workflow reproduces the public MMC1 native result and fails closed with explicit unsupported container/mapper/variant/opcode/control-flow/bank-state/toolchain classifications |
| Private commercial MMC1 compatibility observation | **UNPROVEN** | MMC1 ingestion/platform construction succeeds, but translation stops at opcode byte `0x7C` at `0xC570`, three unresolved `jmp ($E2)` sites and the bank-switched low-window frontier; native execution not reached |
| General NES compatibility | **NOT PROVEN** | No claim for arbitrary NES ROMs, all mappers, commercial-game compatibility, cycle accuracy, full PPU/APU accuracy, FDS or arbitrary 6502 binaries |
...
The Phase-5 NES result is bounded to the audited original Apache-2.0 NROM fixture and the exact deterministic CPU/memory/input/timing/graphics/runtime behaviors exercised by that fixture. It proves that a legally redistributable NES program can cross OpenRecomp's static-recompilation pipeline to generated host-native execution and independently matched observables. It does not establish arbitrary NES ROM compatibility.

The Phase-6 result extends that bounded NES evidence to an original Apache-2.0 MMC1 fixture. The audited claim covers the required MMC1 serial register protocol, PRG/CHR banking, mirroring, native execution and independently structured reference equivalence. The reusable ROM-to-native workflow is likewise bounded: supported inputs can proceed to deterministic native build, while unsupported inputs stop with explicit classifications rather than guessed semantics or targets.

The private commercial MMC1 compatibility fixture remains a compatibility observation only. Phase 6 superseded the earlier MMC1 mapper blocker, but the current frontier remains translation/control-flow: opcode byte `0x7C` at `0xC570`, unresolved `jmp ($E2)` sites at `0x86E8`, `0x8956` and `0x8F3C`, and 1,048 candidate instructions in the bank-switched `$8000-$BFFF` window. Native execution and meaningful interactive behavior have not been demonstrated for this private title, so playability remains **UNPROVEN**.

Phase 7 is the current development track and is intended to close only evidence-supported translation/control-flow gaps: bank-aware code identity/reachability, exact classification of the `0x7C` frontier, and bounded indirect-target recovery. Any later result remains subject to its own audited claim boundary.
...
These results do **not** promote arbitrary RV32I/MIPS32 executables, full MIPS32 ISA/ABI coverage, `div/divu`, unaligned MIPS load/store families, exceptions/coprocessors/floating point, arbitrary NES ROMs, all NES mappers, commercial-game playability, cycle accuracy, full PPU/APU accuracy, FDS compatibility, arbitrary 6502 binaries, macOS/Windows ARM64/Windows x86 ABI parity, arbitrary optimization correctness, WebAssembly AOT compilation, general MIPS32 Unreal hosting, Shipping packaged-build parity, arbitrary Unreal-project/version/platform compatibility or a release-quality production compiler/plugin pipeline to PROVEN.
